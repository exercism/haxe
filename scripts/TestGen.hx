...
This test generator was initially written by David Bruce: https://github.com/davidbruce

His initial PR was stale and his commits were accidentally nuked, hence this attribution.
...
class TestGen
{
	...
	static var exerciseStubTmpl = new Template("package;

class ::exercise:: {
    ::methods:: 
}");

	static var methodsTmpl = new Template("public static function ::method::(::args::): ::returnType:: {
        throw \"Not Implemented\"; // Delete this statement and write your own implementation.
    }");

	static var testTmpl = new Template("package;
...
class Test extends buddy.SingleSuite {
	...
	static var storyTmpl = new Template("describe(\"::story::\", {
        ::testCases::});");

	static var testCaseTmpl = new Template("    it(\"::testCase::\", {
                ::testCriteria::
            });
        ");
	...
	public static function main()
	{
		...
		var execute = (template:Template) -> return template.execute({exercise: exercise});

		var dataJson = Http.requestUrl(execute(dataUrl));
		var descriptionMd = Http.requestUrl(execute(descriptionUrl));
		var metaToml = Http.requestUrl(execute(metaUrl));
		...
		Sys.println('Creating .meta/Example.hx');
		File.saveContent('${dir}.meta/Example.hx', exerciseStubTmpl.execute(
			{
				exercise: toUpperCamel(exercise),
				methods: renderedMethods.join("\n\n\t")
			}));
	...
	private static function generateTests(cases:Array<Dynamic>, methods:Map<String, {order:Int, method:String}>, stories:Array<String>,
			testCases:Map<String, Array<String>>, meta:Array<String>, ?story:String)
	{
		for (testCase in cases)
		{
			if (testCase.cases == null)
			{
				...
				testCases.get(story).push(testCaseTmpl.execute(
					{
	...
	private static function toUpperCamel(string:String, ?seperator:String = ""):String
	{
		...
		for (word in words)
			upperWords.push('${word.substr(0, 1).toUpperCase()}${word.substr(1, word.length)}');
